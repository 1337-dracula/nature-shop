...
export default function PhotoSwiper({ photos, labels }) {
  ...
  return (
    <div
      style={{ position: "relative", overflow: "hidden", height: 400, borderRadius: "16px 16px 0 0", cursor: "grab", userSelect: "none" }}
    ...
    >
      ...
      <div style={{ display: "flex", height: "100%", transform: `translateX(-${current * 100}%)`, transition: "transform 0.3s cubic-bezier(0.4,0,0.2,1)" }}>
        {photos.map((photo, i) => (
          <div key={i} style={{ minWidth: "100%", height: "100%", background: photo.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontSize: 72, gap: 6, padding: "0" }}>
